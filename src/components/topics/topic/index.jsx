...
import { NumberFormat } from "~/utils/formants";
...
export default function Topic({ item }) {
  return (
    ...
    >
      <div className="text-[13px] hover:bg-[color:var(--background-third)] leading-4 ">{item.title}</div>
      ...
      <Popover className="absolute top-1.5 right-2">
        <Popover.Button className=" outline-none w-[34.75px] h-[34.75px] rounded-full text-[color:var(--color-base-secondary)] flex items-center justify-center hover:text-[color:var(--color-primary)] hover:before:hover:bg-[color:var(--color-primary)]  transition-colors  " >
        ...
        <Popover.Panel className=" rounded-2xl grid  absolute top-12 bg-[color:var(--background-primary)] shadow-box shadow-white bg-black right-0 z-50 w-80 h-auto overflow-auto transition-all duration-400">
